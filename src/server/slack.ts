...
// Initialize Slack app with secrets from environment variables
...
export async function verifyRequest(req: NextRequest): Promise<string> {
    ...
    const key = await crypto.subtle.importKey('raw', enc.encode(process.env.SLACK_SIGNING_SECRET), { name: 'HMAC', hash: 'SHA-256' }, false, ['sign', 'verify']);
